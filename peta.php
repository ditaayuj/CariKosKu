...
if($kampus) $where .= " AND k.kampus_terdekat LIKE '%$kampus%'";
...
<form action="" method="GET" class="peta-filter">
    ...
    <select name="kampus">
        ...
        <?php foreach($daftar_kampus as $k): ?>
        <option value="<?= $k ?>" <?= $kampus == $k ? 'selected' : '' ?>><?= $k ?></option>
        <?php endforeach; ?>
    </select>
    <button type="submit" class="peta-btn peta-btn-filter">Tampilkan</button>
    <a href="index_penyewa.php"><button type="button" class="peta-btn peta-btn-back">← Kembali</button></a>
</form>
...
kosData.forEach(function(k){
    ...
    L.marker([k.lat, k.lng], {icon: icon})
        ...
        .bindPopup(
            ...
            '<span style="color:#157A6E;font-weight:700;">Rp'+k.harga+'/bulan</span><br>' +
...
var bounds = L.latLngBounds(kosData.map(function(k){ return [k.lat, k.lng]; }));
